...
//    EmbeddedCuaDriverHost so TCC grants attribute to OperaBot and the
//    driver inherits them. One prompt, named OperaBot, out of the box.
...
const HOST_BUNDLE_ID = "com.operabot.app";
...
async function loadEmbeddedSdk() {
  ...
  process.env.OPERABOT_CUA_SDK_LIBRARY = path.join(
...
async function startEmbedded(binary) {
  ...
  // OperaBot rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart OperaBot`);
  }
...
export async function startCua() {
  ...
  const wantEmbedded =
    app.isPackaged || process.env.OPERABOT_CUA_EMBEDDED === "1";
  let nextConnection;
