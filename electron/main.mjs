...
const DEFAULT_COMPOSIO_BROKER_URL = "https://operabot-composio.nadiasoni201.workers.dev";
...
if (process.platform === "linux") app.setDesktopName("com.operabot.app.desktop");
...
async function secureComposioConfig() {
  const dataDir = process.env.OMB_DATA_DIR || path.join(app.getPath("home"), ".operabot");
...
// goes to server.log in the OS log dir (~/Library/Logs/OperaBot on macOS,
// Console.app-visible; %APPDATA%\OperaBot\logs on Windows), which is also
  ...
  for (let i = 0; i < 40; i++) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (body?.app === "operabot" && body.pid === proc.pid && body.static) return proc;
...
const ERROR_PAGE =
  ...
  encodeURIComponent(
    `<body style="margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#070707;color:#fcfcfc;font:15px -apple-system,system-ui"><div style="text-align:center;max-width:360px"><div style="font-size:40px">🐭</div><h2 style="font-weight:600;margin:12px 0 6px">Couldn't start the bot server</h2><p style="color:#fcfcfc99;line-height:1.5">Something else is using its ports. Quit and reopen OperaBot — if it keeps happening, restart your computer.</p></div></body>`,
  );
