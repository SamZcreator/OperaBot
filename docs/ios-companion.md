...
The iOS app is a thin, native client for the OperaBot instance running on
...
The sidecar keeps its device registry in `~/.operabot/devices.json`. That is
...
The sidecar advertises `_operabot._tcp` over Bonjour. The app browses with
...
Tailscale is optional. There is no OperaBot-operated relay or cloud copy of
...
one-time exchange for a securely stored long-lived credential. An OperaBot
