...
OperaBot has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
installed builds do not require Node, pnpm, Swift, or a terminal at runtime.

## What works

- The native Electron window and embedded OperaBot server on GNOME Xorg and GNOME Wayland.
...
ARM64 are follow-ups in [issue #29](https://github.com/SamZcreator/OperaBot/issues/29).
...
```sh
git clone https://github.com/SamZcreator/OperaBot.git
cd OperaBot
corepack enable
...
- `release/OperaBot-<version>-amd64.deb`
- `release/OperaBot-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./release/OperaBot-*-amd64.deb
```

Then open **OperaBot** from the GNOME application launcher. To remove it:

```sh
sudo apt remove operabot
```

The portable AppImage does not install system files:

```sh
chmod +x release/OperaBot-*-x86_64.AppImage
./release/OperaBot-*-x86_64.AppImage
```

Application data remains local in `~/.operabot`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/operabot` unless the environment overrides it).
...
./release/linux-unpacked/operabot
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. OperaBot
...
```sh
OMB_EXTRA_PATH=/your/custom/bin ./release/OperaBot-*-x86_64.AppImage
```

Restart OperaBot after installing or signing in to a CLI.

## Xorg and Wayland

The baseline shell, chat, cloud computers, and connected apps work in both GNOME session types. OperaBot
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart OperaBot. If it lives outside the
...
```sh
chmod +x OperaBot-*-x86_64.AppImage
file OperaBot-*-x86_64.AppImage
```
