# OperaBot harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what OperaBot lacked by
...
**The reframe that reorders everything.** OperaBot is itself a harness. The
...
**On-disk forward compatibility.** `~/.operabot` is shared across app versions.
