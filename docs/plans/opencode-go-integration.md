...
Add OpenCode Go as a first-class OperaBot engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse OperaBot's ACP runtime rather than call
...
provide inference; OperaBot does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. OperaBot should
...
the process lifecycle OperaBot uses for Grok and Gemini.
...
- load that session on the next OperaBot turn;
...
- concurrent OperaBot threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; OperaBot never selects it as a runnable default otherwise.
- The setup flow works without restarting OperaBot and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so OperaBot does not double-count it?
