# Computer use & browser use in OperaBot

Decision doc, 2026-08-12. How bots in OperaBot get local computer use and
...
│     one TCC prompt, named OperaBot          │ unix socket (private)
...
**Decision (Nadia, 2026-08-12): CUA is the ONLY computer-use provider.
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is OperaBot's, users see ONE
   prompt named OperaBot, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `OperaBot.app/Contents/Resources/cua-driver`,
