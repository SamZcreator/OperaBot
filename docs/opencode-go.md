...
OpenCode Go is an optional OperaBot engine. OperaBot runs the maintained
...
3. Open OperaBot Settings → Connections and save the key under **OpenCode
   Go API key**.

The key is stored locally as write-only configuration. OperaBot reports only
whether it is configured, never the value. A key saved in OperaBot is
...
`opencode auth login`; OperaBot does not edit OpenCode auth/config files.
