...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "operabot", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "OperaBot-packager" } });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "operabot-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __operabotCreateRequire } from "node:module"; const require = __operabotCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
...
await writeFile(
  ...
  bundledSource.replace(
    resolverPattern,
    `${resolvers[0]}\n      if (process.env.OPERABOT_CUA_SDK_LIBRARY) return resolveOverride(opts.crateName, process.env.OPERABOT_CUA_SDK_LIBRARY);`,
  ),
);
