...
const executable = path.resolve(
  process.env.OMB_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "operabot"),
);
...
mkdirSync(path.join(home, ".operabot"), { recursive: true });
mkdirSync(xdgConfig, { recursive: true });
writeFileSync(
  path.join(home, ".operabot", "config.json"),
...
try {
  ...
  if (health?.app !== "operabot" || health.static !== true) {
    throw new Error(`unexpected embedded health response: ${JSON.stringify(health)}`);
  }
  if (!String(title).includes("OperaBot")) throw new Error(`unexpected renderer title: ${title}`);
