...
export class RoutineManager {
    ...
    disableForBot(botId) {
        ...
        for (const run of this.runs) {
            ...
            this.emitRun(run);
            if (run.threadId)
                void this.options.interruptTurn?.(run.botId, run.threadId, run.runOn ?? "masque").catch(() => { });
            changed = true;
        }
        if (changed)
            this.save();
    ...
    async cancelRun(id) {
        ...
        this.emitRun(run);
        if (run.threadId)
            await this.options.interruptTurn?.(run.botId, run.threadId, run.runOn ?? "masque").catch(() => { });
        queueMicrotask(() => void this.tick());
        return { ...run };
    }
    markSeen(id) {
        ...
        try {
            ...
            for (const run of [...this.runs].reverse()) {
                ...
                try {
                    ...
                    await this.options.startTurn(run.botId, task.threadId, prompt, run.runOn ?? "masque", triggerSource, (message) => this.failThread(task.threadId, message));
                }
                catch (error) {
