...
export const TEAM_MANIFEST_FORMAT = "operabot.team";
export const TEAM_MANIFEST_VERSION = 2;
export const LEGACY_TEAM_MANIFEST_VERSION = 1;
...
const requiredText = (max) => z.string({ error: "must be text" }).trim().min(1, { message: "is required" }).max(max, { message: "is too long" });
...
const manifestSchema = z.discriminatedUnion("version", [
    z.object({
        format: z.literal(TEAM_MANIFEST_FORMAT, { error: "This is not an OperaBot team file" }),
...
export function parseTeamManifest(value) {
    ...
    if (!parsed.success) {
        ...
        if (formatIssue)
            throw new Error(formatIssue.message);
        const versionIssue = parsed.error.issues.find((issue) => issue.path[0] === "version");
        if (versionIssue)
            throw new Error("Team file version is not supported");
        throw new Error(schemaIssue(parsed.error, "This is not a team file"));
    }
    const seenKeys = new Set();
    const members = parsed.data.team.members.map((member) => {
        if (seenKeys.has(member.key))
            throw new Error(`Duplicate member key: ${member.key}`);
        seenKeys.add(member.key);
        const appearance = { color: member.appearance.color };
        if (member.appearance.mascotExpression)
            appearance.mascotExpression = member.appearance.mascotExpression;
        return {
            key: member.key,
            name: member.name,
            title: member.title ?? "",
    ...
    if (parsed.data.team.description)
        result.team.description = parsed.data.team.description;
...
export function createTeamManifest(team, bots) {
    ...
    const members = team.memberIds.map((id, index) => {
        ...
        const appearance = { color: bot.color };
        if (bot.mascotExpression)
            appearance.mascotExpression = bot.mascotExpression;
        return {
            key,
            name: bot.name,
            title: bot.title,
