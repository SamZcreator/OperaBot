// Config + data dirs. One file, ~/.operabot/config.json, env fallbacks:
...
export function parseStoredConfig(value) {
    const parsed = appConfigSchema.safeParse(value);
    if (!parsed.success)
        throw new Error(schemaIssue(parsed.error, "Invalid stored configuration"));
    return parsed.data;
}
...
export const DATA_DIR = process.env.OMB_DATA_DIR ?? join(homedir(), ".operabot");
...
export function loadConfig() {
    ...
    if (process.env.COMPOSIO_API_KEY !== undefined)
        cfg.composio.apiKey = process.env.COMPOSIO_API_KEY;
    cfg.box = { token: process.env.BOX_TOKEN, ...cfg.box };
    cfg.opencodeGo = { apiKey: process.env.OPENCODE_API_KEY, ...cfg.opencodeGo };
    cfg.tts = { key: process.env.OMB_TTS_KEY, ...cfg.tts };
    return cfg;
}
/** Merge a partial config into ~/.operabot/config.json (secrets never
...
export function saveConfig(patch) {
    ...
    if (checkedPatch.instances) {
        ...
        const diskInstances = currentInstances.success ? currentInstances.data : {};
...
export function withInstanceCli(cfg, instanceId, cli) {
    ...
    if (cliKey) {
        const nextConfig = currentConfig.success ? { ...currentConfig.data } : {};
...
function injectedEnvironment(cfg) {
    ...
    if (cfg.xai?.key)
        environment.set("XAI_API_KEY", cfg.xai.key);
    if (cfg.box?.token)
        environment.set("BOX_TOKEN", cfg.box.token);
    if (cfg.opencodeGo?.apiKey)
        environment.set("OPENCODE_API_KEY", cfg.opencodeGo.apiKey);
    return environment;
}
...
export function instanceConfigs(cfg) {
    ...
    // a credential Nadia doesn't want to manage; an `instances` entry brings
    // it back anytime.
    //
    ...
    for (const entry of Object.values(map)) {
        ...
        if (cfg.xai?.key)
            environment.XAI_API_KEY = cfg.xai.key;
        if (cfg.box?.token)
            environment.BOX_TOKEN = cfg.box.token;
