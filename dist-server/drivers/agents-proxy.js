...
var HARNESS = process.env.OMB_HARNESS_URL ?? "http://127.0.0.1:8799";
var BOT_ID = process.env.OMB_BOT_ID ?? "";
var THREAD_ID = process.env.OMB_THREAD_ID ?? "";
var TOKEN = process.env.OMB_COMMS_TOKEN ?? "";
var DEPTH = Number(process.env.OMB_TURN_DEPTH ?? "0") || 0;
var TOOLS = [
  {
    name: "list_bots",
    description: "List the other bots (agents) in this OperaBot workspace you can message, with their model and whether they're busy. Call this before ask_bot to discover who's available.",
...
var rpcErr = (id, code, message) => send({ jsonrpc: "2.0", id, error: { code, message } });
var textResult = (id, text, isError = false) => ok(id, { content: [{ type: "text", text }], isError });
...
async function handle(msg) {
  ...
  switch (method) {
    ...
    default:
      if (id !== void 0) rpcErr(id, -32601, `Method not found: ${method}`);
  }
}
...
rl.on("line", (line) => {
  ...
  void handle(msg).catch((e) => {
    if (msg.id !== void 0) rpcErr(msg.id, -32603, e.message);
  });
});
