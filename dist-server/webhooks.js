...
import { parseJson, schemaIssue } from "./schema.js";
...
const runOnSchema = z.enum(["masque", "cloud"]);
...
function cleanInput(input) {
    ...
    const runOn = input.runOn ?? "masque";
    if (!name)
        fail(400, "Give the webhook a name");
    if (!botId)
        fail(400, "Choose a MASQUE");
    if (runOn !== "masque" && runOn !== "cloud")
...
function parseTriggerPatch(value) {
...
export class WebhookManager {
    ...
    create(input) {
        ...
        if (this.options.botState(clean.botId) === "missing")
            fail(400, "That MASQUE no longer exists");
        const now = this.now();
    ...
    disableForBot(botId) {
        ...
        for (const trigger of this.webhooks) {
            ...
            trigger.updatedAt = this.now();
            this.options.cancelQueued?.(trigger.id, "The assigned MASQUE was deleted");
            this.emit(trigger);
            changed = true;
        }
        if (changed)
    ...
    test(id, payload = { event: "operabot.test", message: "Test webhook delivery" }) {
        const trigger = this.webhooks.find((candidate) => candidate.id === id);
        if (!trigger)
            return null;
        const eventName = trigger.eventTypes?.[0] ?? "operabot.test";
        return this.dispatch(trigger, {
            payload,
            contentType: "application/json",
            eventName,
            userAgent: "OperaBot webhook tester",
    ...
    dispatch(trigger, event) {
        ...
        if (this.options.botState(trigger.botId) === "missing")
            fail(410, "The assigned MASQUE no longer exists");
    ...
    captureVerification(trigger, event) {
        ...
        if (event.eventName)
            sample.eventName = event.eventName.slice(0, 200);
        if (event.contentType)
            sample.contentType = event.contentType.slice(0, 200);
        trigger.verificationSample = sample;
        this.appendAttempt(trigger, event, {
            outcome: "captured",
            statusCode: 202,
            deliveryId,
            reason: "Test event captured; enable the webhook to start MASQUE tasks",
        });
        this.save();
        this.emit(trigger);
    ...
    appendAttempt(trigger, event, details) {
        ...
        if (event.eventName)
            attempt.eventName = event.eventName.slice(0, 200);
        if (event.payload !== undefined)
            attempt.preview = previewPayload(event.payload);
        if (details.deliveryId)
            attempt.deliveryId = details.deliveryId.slice(0, 200);
        if (details.runId)
            attempt.runId = details.runId;
        if (details.reason)
            attempt.reason = details.reason;
