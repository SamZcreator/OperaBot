...
import { MasqueAvatar, InitialsAvatar } from "./Avatar";
...
/** "Nadia Vos" → "MS", "nadia" → "M", "you@x.dev" → "Y", unset → "?" */
...
/** Room avatar: 2–3 overlapping masquees in the same 56px slot a bot gets. */
function StackedMasquees({ members }: { members: Bot[] }) {
  if (members.length <= 1) {
    const b = members[0];
    return (
      <div className="flex size-14 shrink-0 items-center justify-center">
        {b ? <MasqueAvatar color={b.color} state="happy" size={56} /> : <Users size={24} className="text-ink-secondary" />}
      </div>
    );
  }
  ...
  return (
    <div className="flex size-14 shrink-0 items-center justify-center">
      <div className="flex items-center -space-x-3">
        {shown.map((b) => (
          <MasqueAvatar key={b.id} color={b.color} state="happy" size={30} />
        ))}
        {extra > 0 && (
...
function GroupListItem({ group, onMenu }: { group: Group; onMenu: (menu: { groupId: string; x: number; y: number }) => void }) {
  ...
  return (
    ...
    >
      <StackedMasquees members={members} />
  ...
  return (
    ...
    >
      <div className="w-[340px] rounded-2xl border border-hairline/50 bg-card p-4 shadow-2xl">
        <div className="mb-3 text-[15px] font-semibold text-ink">{t("room.new")}</div>
        <input
          autoFocus
          value={name}
        ...
        <div className="flex max-h-64 flex-col gap-0.5 overflow-y-auto">
          {bots.length === 0 && (
            <div className="px-2 py-4 text-center text-[13px] text-ink-secondary">{t("room.needsBot")}</div>
          )}
          {bots.map((b) => (
            <button
              key={b.id}
            ...
            >
              <MasqueAvatar color={b.color} state="happy" size={28} />
    ...
    >
      {[
        ...
        item(
          ...
          bot.chiefOfStaff ? t("chief.remove") : t("chief.make"),
          () => dispatch({ type: "updateBot", botId: bot.id, patch: { chiefOfStaff: !bot.chiefOfStaff } }),
          {
            disabled: !bot.chiefOfStaff && !canCoordinate,
            hint: !bot.chiefOfStaff && !canCoordinate ? t("engine.chooseFirst") : undefined,
          },
        ),
        ...
        divider("d1"),
        item(<Pencil size={16} className="text-ink-secondary" />, t("profile.edit"), () => {
        ...
        divider("d2"),
        item(<ClipboardCopy size={16} className="text-ink-secondary" />, t("room.copyId"), () => {
    ...
    >
      ...
      >
        <header className="flex items-start justify-between gap-4 px-6 pb-4 pt-6 sm:px-8 sm:pt-7">
          <div>
            <h2 id="archived-bots-title" className="text-[22px] font-semibold tracking-[-0.01em] text-ink">{t("archive.title")}</h2>
            <p className="mt-1 text-[13px] text-ink-secondary">{t("archive.hint")}</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-x-8 md:grid-cols-2">
            {bots.map((bot) => (
              <div key={bot.id} className="flex min-h-[82px] items-center gap-3 border-b border-hairline/35 px-1 py-3">
                <MasqueAvatar color={bot.color} state="happy" size={42} />
                <div className="min-w-0 flex-1">
    ...
    >
            ...
            <>
              ...
              <div className="absolute right-0 top-full z-40 mt-1 w-44 overflow-hidden rounded-xl border border-hairline/50 bg-card py-1.5 shadow-2xl shadow-black/60">
                ...
                {archivedBots.length > 0 && (
                  ...
                  >
                    <Archive size={16} className="text-ink-secondary" />
                    <span className="flex-1">{t("archive.title")}</span>
      ...
      <div className="px-3 pb-3 pt-2">
        ...
        >
          ...
          <span className="flex-1 text-[14px]">{t("nav.automations")}</span>
        ...
        >
          ...
          <span className="text-[14px] text-ink">{t("settings.connectedApps")}</span>
        </button>
