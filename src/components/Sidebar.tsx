...
function importPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format !== "operabot.team") throw new Error("This is not an OperaBot team file.");
        ...
        <div className="mt-3 flex max-h-64 flex-col gap-0.5 overflow-y-auto rounded-xl bg-raised/30 p-1.5">
          ...
          {bots.map((bot) => {
            ...
            return (
              ...
              >
                <MasqueAvatar color={bot.color} state="happy" size={28} />
  ...
  return (
    ...
    >
      ...
      <input
        ref={importInputRef}
        type="file"
        accept=".json,.masqueteam.json,application/json"
        onChange={(event) => void chooseTeamFile(event)}
        className="hidden"
        aria-label="Choose an OperaBot team file"
      />
