      ...
      <div className="max-h-[90vh] w-full max-w-[620px] overflow-y-auto rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-hairline/40 bg-panel/95 px-5 py-4 backdrop-blur">
          <div>
            ...
            <div className="mt-0.5 text-[12px] text-ink-secondary">Give a MASQUE scheduled work with a calendar you can trust.</div>
          </div>
        ...
        <div className="space-y-5 p-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">Uses this MASQUE's selected model and computer setting.</div>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">Runs the MASQUE and its tools inside its Box virtual machine.</div>
              </button>
            </div>
            {runOn === "cloud" && (
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-medium text-ink-secondary">What should this MASQUE do?</span>
        ...
        <div className="max-h-[55vh] space-y-4 overflow-y-auto p-5">
          {routine && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">Runs on</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{routine.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{routine.runOn === "cloud" ? "Cloud VM" : "MASQUE setup"}</div></div>
          ...
          {run?.triggerSource === "webhook" && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">Runs on</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{run.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{run.runOn === "cloud" ? "Cloud VM" : "MASQUE setup"}</div></div>
          ...
          {run?.status === "waiting" && <div className="rounded-xl border border-warning/30 bg-warning/10 px-3.5 py-3 text-[13px] text-warning">This MASQUE needs your answer. Open its task to continue the run.</div>}
        </div>
          ...
          {routines.map((routine) => {
            ...
            return (
              <div key={routine.id} className="flex items-center gap-3 rounded-xl border border-hairline/40 bg-inset p-3">
                ...
                <div className="min-w-0 flex-1"><div className="truncate text-[14px] font-semibold text-ink">{routine.name}</div><div className="mt-0.5 truncate text-[11.5px] text-ink-secondary">{bot?.name ?? "Deleted MASQUE"} · {scheduleLabel(routine)}</div></div>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12.5px] text-ink-secondary">{section === "calendar" ? "Run MASQUE tasks on a schedule." : "Run MASQUE tasks when an event arrives."}</p>
          </div>
        ...
        {section === "calendar" && <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <select value={botFilter} onChange={(event) => setBotFilter(event.target.value)} className="rounded-xl border border-hairline/50 bg-panel px-3 py-2 text-[12px] text-ink outline-none focus:border-accent/60">
            <option value="all">All MASQUEes</option>
      ...
      {section === "webhooks" ? (
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center p-8">
          <div className="max-w-[430px] text-center">
            ...
            <h2 className="text-[18px] font-semibold text-ink">Put your MASQUE team on a rhythm</h2>
