...
import { MASQUE_COLORS, type MasqueState } from "@/lib/mascot";
...
function statusState(status: RoutineRunStatus): MasqueState {
  switch (status) {
    case "queued":
      return "drowsy";
    case "running":
...
function RoutineCard({ item, bot, compact, onOpen }: { item: CalendarItem; bot: Bot; compact: boolean; onOpen: () => void }) {
  ...
  const color = MASQUE_COLORS[bot.color];
      ...
      <div className={cn("flex min-w-0 items-center", compact ? "gap-1.5" : "gap-2")}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center gap-1.5 truncate text-[10.5px] text-white/70">
            ...
            <span>·</span>
            {item.run?.triggerSource === "webhook" && <><Webhook size={10} /><span>{t("routine.webhook")}</span><span>·</span></>}
...
export function RoutineEditor({
  ...
  const [runOn, setRunOn] = useState<RoutineRunOn>(routine?.runOn ?? defaultRunOn ?? "masque");
      ...
      <div className="max-h-[90vh] w-full max-w-[620px] overflow-y-auto rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-hairline/40 bg-panel/95 px-5 py-4 backdrop-blur">
          <div>
            ...
            <div className="mt-0.5 text-[12px] text-ink-secondary">{t("routine.intro")}</div>
          </div>
        ...
        <div className="space-y-5 p-5">
          <label className="block">
            ...
            <input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder={t("routine.namePlaceholder")} className="w-full rounded-xl border border-hairline/60 bg-inset px-3.5 py-2.5 text-[14px] text-ink outline-none placeholder:text-ink-secondary/60 focus:border-accent/70" />
          </label>
          <div>
            <div className="mb-2 text-[12px] font-medium text-ink-secondary">{t("routine.whereRuns")}</div>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setRunOn("masque")}
                className={cn(
                  "rounded-xl border p-3 text-left transition",
                  runOn === "masque" ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-inset hover:bg-raised/60",
                )}
              >
                <div className="flex items-center gap-2 text-[13px] font-medium text-ink"><Laptop size={15} />{t("routine.thisComputer")}</div>
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">{t("routine.thisComputerHint")}</div>
              </button>
              <button
                type="button"
              ...
              >
                <div className="flex items-center gap-2 text-[13px] font-medium text-ink"><Cloud size={15} />{t("routine.cloudVm")}</div>
                <div className="mt-1 text-[11px] leading-relaxed text-ink-secondary">{t("routine.cloudVmHint")}</div>
              </button>
            </div>
            {runOn === "cloud" && (
              <div className={cn("mt-2 rounded-lg px-3 py-2 text-[11.5px] leading-relaxed", cloudReady ? "bg-accent/10 text-ink-secondary" : "border border-warning/25 bg-warning/10 text-warning")}>
                {cloudReady
                  ? "The VM wakes automatically for each run. Keep OperaBot running so its scheduler can launch the job."
                  : "Cloud VM needs a working Box API key in App Settings before this routine can run."}
              </div>
            )}
          </div>
          <div>
            <div className="mb-2 text-[12px] font-medium text-ink-secondary">{t("routine.whoDoes")}</div>
            <div className={cn("grid gap-2", lockedBotId ? "grid-cols-1" : "grid-cols-2 sm:grid-cols-3")}>
              {bots.map((bot) => (
                <button key={bot.id} type="button" disabled={Boolean(lockedBotId)} onClick={() => setBotId(bot.id)} className={cn("flex min-w-0 items-center gap-2 rounded-xl border px-3 py-2 text-left", botId === bot.id ? "border-accent/70 bg-accent/10" : "border-hairline/50 bg-inset hover:bg-raised/60")}>
                  <MasqueAvatar color={bot.color} state={botId === bot.id ? "happy" : "idle"} size={38} animated={false} />
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-medium text-ink-secondary">{t("routine.whatDo")}</span>
          ...
          <div>
            <div className="mb-2 text-[12px] font-medium text-ink-secondary">{t("routine.when")}</div>
          ...
          <label className="block">
            <span className="mb-1.5 block text-[12px] font-medium text-ink-secondary">{t("routine.calendarBlock")}</span>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-5 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="w-full max-w-[520px] overflow-hidden rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="relative overflow-hidden border-b border-hairline/40 px-5 py-5" style={{ background: `linear-gradient(135deg, color-mix(in srgb, ${MASQUE_COLORS[bot.color]} 28%, #111), #111)` }}>
          ...
          <div className="flex items-center gap-4 pr-10">
            <MasqueAvatar color={bot.color} state={run ? statusState(run.status) : "idle"} size={72} animated={run?.status === "running" || run?.status === "waiting"} label={bot.name} />
        ...
        <div className="max-h-[55vh] space-y-4 overflow-y-auto p-5">
          {routine && (
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("routine.schedule")}</div><div className="mt-1 text-[13px] text-ink">{scheduleLabel(routine)}</div></div>
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("routine.runsOn")}</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{routine.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{routine.runOn === "cloud" ? t("routine.cloudVm") : t("routine.botSetup")}</div></div>
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("routine.duration")}</div><div className="mt-1 text-[13px] text-ink">{routine.durationMinutes} minutes</div></div>
            </div>
          )}
          {run?.triggerSource === "webhook" && (
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("routine.triggeredBy")}</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink"><Webhook size={13} />{t("routine.webhook")}</div></div>
              <div className="rounded-xl bg-inset p-3"><div className="text-[10px] uppercase tracking-wider text-ink-secondary">{t("routine.runsOn")}</div><div className="mt-1 flex items-center gap-1.5 text-[13px] text-ink">{run.runOn === "cloud" ? <Cloud size={13} /> : <Laptop size={13} />}{run.runOn === "cloud" ? t("routine.cloudVm") : t("routine.botSetup")}</div></div>
          ...
          {run?.status === "waiting" && <div className="rounded-xl border border-warning/30 bg-warning/10 px-3.5 py-3 text-[13px] text-warning">{t("routine.needsAnswer")}</div>}
        </div>
        <div className="flex flex-wrap items-center gap-2 border-t border-hairline/40 px-5 py-4">
          ...
          {routine && <button onClick={() => onEdit(routine)} className="rounded-xl px-3 py-2 text-[13px] text-ink-secondary hover:bg-raised hover:text-ink">{t("routine.edit")}</button>}
          {routine && <button onClick={() => { if (!window.confirm(`Delete “${routine.name}”? Its past run receipts will stay in the calendar.`)) return; dispatch({ type: "deleteRoutine", routineId: routine.id }); onClose(); }} className="rounded-xl p-2 text-ink-secondary hover:bg-danger/10 hover:text-danger" title={t("routine.delete")}><Trash2 size={16} /></button>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-5 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="w-full max-w-[560px] overflow-hidden rounded-2xl border border-hairline/60 bg-panel shadow-2xl">
        <div className="flex items-center justify-between border-b border-hairline/40 px-5 py-4">
          <div><div className="text-[17px] font-semibold text-ink">{t("routine.paused")}</div><div className="mt-0.5 text-[12px] text-ink-secondary">{t("routine.pausedHint")}</div></div>
          <button onClick={onClose} className="rounded-lg p-2 text-ink-secondary hover:bg-raised hover:text-ink"><X size={18} /></button>
        </div>
          ...
          {routines.map((routine) => {
            ...
            return (
              <div key={routine.id} className="flex items-center gap-3 rounded-xl border border-hairline/40 bg-inset p-3">
                {bot ? <MasqueAvatar color={bot.color} state="sleeping" size={44} animated={false} label={bot.name} /> : <div className="flex size-11 items-center justify-center rounded-xl bg-raised text-ink-secondary"><CalendarClock size={20} /></div>}
                <div className="min-w-0 flex-1"><div className="truncate text-[14px] font-semibold text-ink">{routine.name}</div><div className="mt-0.5 truncate text-[11.5px] text-ink-secondary">{bot?.name ?? t("routine.deletedBot")} · {scheduleLabel(routine)}</div></div>
                ...
                <button onClick={() => onEdit(routine)} className="rounded-lg px-2 py-1.5 text-[12px] text-ink-secondary hover:bg-raised hover:text-ink">{t("routine.edit")}</button>
                <button onClick={() => { if (!window.confirm(`Delete “${routine.name}”?`)) return; dispatch({ type: "deleteRoutine", routineId: routine.id }); }} className="rounded-lg p-2 text-ink-secondary hover:bg-danger/10 hover:text-danger" title={t("routine.delete")}><Trash2 size={15} /></button>
              </div>
            );
          })}
        </div>
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[12.5px] text-ink-secondary">{section === "calendar" ? t("routine.calendarIntro") : t("routine.webhookIntro")}</p>
          </div>
        ...
        <div className="mt-4 flex items-center gap-1 rounded-xl bg-panel p-1 sm:w-fit">
          <button onClick={() => setSection("calendar")} className={cn("flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[12px] font-medium", section === "calendar" ? "bg-raised text-ink shadow" : "text-ink-secondary hover:text-ink")}><CalendarDays size={13} />{t("routine.schedules")}</button>
        ...
        {section === "calendar" && <div className="mt-3 flex flex-wrap items-center gap-2">
          <div className="flex items-center rounded-xl border border-hairline/50 bg-panel p-0.5">
            <button onClick={() => move(-1)} className="rounded-lg p-2 text-ink-secondary hover:bg-raised hover:text-ink" aria-label={t("routine.previousDates")}><ChevronLeft size={16} /></button>
            <button onClick={goToday} className="px-2.5 py-1.5 text-[12px] font-medium text-ink hover:text-accent">{t("routine.today")}</button>
            <button onClick={() => move(1)} className="rounded-lg p-2 text-ink-secondary hover:bg-raised hover:text-ink" aria-label={t("routine.nextDates")}><ChevronRight size={16} /></button>
          </div>
          ...
          <select value={botFilter} onChange={(event) => setBotFilter(event.target.value)} className="rounded-xl border border-hairline/50 bg-panel px-3 py-2 text-[12px] text-ink outline-none focus:border-accent/60">
            <option value="all">{t("routine.allBots")}</option>
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center p-8">
          <div className="max-w-[430px] text-center">
            <div className="relative mx-auto mb-5 flex h-28 w-44 items-end justify-center">
              {visibleBots.slice(0, 3).map((bot, index) => <div key={bot.id} className="-ml-3 first:ml-0" style={{ transform: `translateY(${Math.abs(index - 1) * 9}px) rotate(${(index - 1) * 5}deg)` }}><MasqueAvatar color={bot.color} state={index === 1 ? "excited" : "idle"} size={84} /></div>)}
              {visibleBots.length === 0 && <CalendarClock size={58} className="text-ink-secondary/40" />}
            </div>
            <h2 className="text-[18px] font-semibold text-ink">{t("routine.empty")}</h2>
            ...
            <button onClick={() => setEditor("new")} disabled={visibleBots.length === 0} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-[13px] font-medium text-white hover:brightness-110 disabled:opacity-40"><Plus size={15} />{t("routine.createFirst")}</button>
