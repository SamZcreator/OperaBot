...
// in the app's historical MasqueAvatar API so no call site changes: per-bot
...
// hand-built Masque body + face engine (masque-engine/face/driver) is gone;
...
import { MASQUE_COLORS, type MasqueColor, type MasqueMotion, type MasqueState } from "@/lib/mascot";
...
/**
 * Legacy face-placement knobs from the Masque body era. The cursor mascot
...
const MOTION_FACE: Partial<
  Record<Exclude<MasqueMotion, "none">, { state?: MasqueState; blink?: boolean; spin?: number }>
> = {
 ...
 */
const gradientFor = (color: MasqueColor): [string, string, string] => {
  const fill = MASQUE_COLORS[color] ?? MASQUE_COLORS.green;
  return [mix(fill, "#ffffff", 0.55), fill, mix(fill, "#000000", 0.42)];
};

export type MasqueAvatarHandle = CursorAvatarHandle;

export type MasqueAvatarProps = {
  color: MasqueColor;
  ...
  animated?: boolean;
  /** Legacy Masque face-placement knobs — accepted, ignored. */
  eyeSpacing?: number;
  faceX?: number;
  faceY?: number;
  faceScale?: number;
...
function MasqueAvatarComponent(
  {
    color,
    state = "idle",
    expression,
  ...
  ref: React.Ref<MasqueAvatarHandle>,
) {
  ...
  const [motionState, setMotionState] = useState<MasqueState | null>(null);
  useEffect(() => {
  ...
  return (
    ...
    >
      <CursorAvatar
        ...
        eyeColor={mix(MASQUE_COLORS[color] ?? MASQUE_COLORS.green, "#000000", 0.6)}
        title={label ?? null}
...
export const MasqueAvatar = memo(forwardRef(MasqueAvatarComponent));
