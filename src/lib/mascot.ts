...
export type MasqueState = CursorState;
export const MASQUE_STATES = CURSOR_STATES;
...
export type MasqueMotion = "none" | (typeof MASQUE_MOTIONS)[number];
...
const KNOWN_STATES = new Set<string>(MASQUE_STATES);

/** Resolves any stored value — current, legacy or junk — to a real state. */
export function normalizeState(value: string | null | undefined): MasqueState | null {
  if (!value) return null;
  if (KNOWN_STATES.has(value)) return value as MasqueState;
 ...
 */
export const PICKABLE_STATES: MasqueState[] = [
  "idle", // expression 0
  "happy", // 2
  "curious", // 3
  "drowsy", // 4
 ...
 */
export function stateForBot(bot: MascotBotProfile): MasqueState {
