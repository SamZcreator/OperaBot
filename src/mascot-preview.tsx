...
function Tuner({
  ...
  state: MasqueState;
  setState: (s: MasqueState) => void;
  expression: number | undefined;
  setExpression: (e: number | undefined) => void;
  color: MasqueColor;
  setColor: (c: MasqueColor) => void;
  forward: boolean;
  setForward: (v: boolean) => void;
}) {
  const handle = useRef<MasqueAvatarHandle>(null);
          ...
          <div className="chips">
            {MASQUE_COLOR_NAMES.map((c) => (
              <button
                ...
                onClick={() => setColor(c)}
              >
                <span className="swatch" style={{ background: MASQUE_COLORS[c] }} />
                {c}
              </button>
            ))}
          </div>
...
function MotionCard({
  ...
  motion: Exclude<MasqueMotion, "none">;
  index: number;
  replayAll: number;
  color: MasqueColor;
}) {
...
function Preview() {
  ...
  const [state, setState] = useState<MasqueState>("idle");
  const [expression, setExpression] = useState<number | undefined>(undefined);
  const [color, setColor] = useState<MasqueColor>("green");
  ...
  return (
    <main className="preview-shell">
      ...
      <section className="motion-grid" aria-label="Mascot motion library">
        {MASQUE_MOTIONS.map((motion, index) => (
          <MotionCard
            key={motion}
            motion={motion}
            index={index}
      ...
      <section className="expression-library" aria-labelledby="expression-heading">
        <div className="section-heading">
          ...
          <p>Move your pointer over any Masque to test the responsive eyes.</p>
        </div>
            ...
            {MASQUE_COLOR_NAMES.map((c) => (
              <div className="matrix-row" key={c}>
                <div className="row-label">
                  <span className="swatch" style={{ background: MASQUE_COLORS[c] }} />
                  <strong>{c}</strong>
                  <code>{MASQUE_COLORS[c]}</code>
                </div>
                {PICKABLE_STATES.map((s) => (
                  <div className="mascot-cell" key={`${c}-${s}`}>
                    <MasqueAvatar color={c} state={s} size={86} label={`${c} ${s} masque`} />
                  </div>
                ))}
              </div>
            ))}
