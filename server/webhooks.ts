...
function cleanInput(input: WebhookTriggerInput): CleanWebhookInput {
  ...
  if (!botId) fail(400, "Choose a MASQUE");
  if (runOn !== "masque" && runOn !== "cloud") fail(400, "Choose where this webhook runs");
...
export class WebhookManager {
  ...
  create(input: JsonValue): CreatedWebhook {
    ...
    if (this.options.botState(clean.botId) === "missing") fail(400, "That MASQUE no longer exists");
    const now = this.now();
  ...
  test(id: string, payload: JsonValue = { event: "operabot.test", message: "Test webhook delivery" }): WebhookReceiveResult | null {
    const trigger = this.webhooks.find((candidate) => candidate.id === id);
    if (!trigger) return null;
  ...
  private dispatch(trigger: StoredWebhookTrigger, event: WebhookEvent): WebhookReceiveResult {
    ...
    if (this.options.botState(trigger.botId) === "missing") fail(410, "The assigned MASQUE no longer exists");
