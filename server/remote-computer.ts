...
// inside the VM and OperaBot never exposes another inbound port.
...
export const REMOTE_CUA_SESSION = "operabot";
export const REMOTE_CDP_HELPER = "/opt/ogb/operabot-cdp.mjs";
...
export function remoteComputerBootstrapCommand(botName: string): string {
  ...
  return [
    ...
    `tmux has-session -t work 2>/dev/null || tmux new-session -d -s work 'echo; echo "  ▦ ${safeName}'"'"'s computer — OperaBot"; echo; exec bash -i'`,
    "echo bootstrapped",
  ].join("\n");
}
