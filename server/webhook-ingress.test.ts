...
beforeAll(async () => {
  ...
  const created = manager.create({ name: "Build event", prompt: "Review the build", botId: "masque-1" });
...
describe("webhook-only ingress", () => {
  it("exposes health but nothing from the main OperaBot API", async () => {
    const health = await fetch(`${ingress.baseUrl}/health`);
    expect(health.status).toBe(200);
    expect(await health.json()).toEqual({ app: "operabot-webhooks", ready: true });
  ...
  it("captures a verification event without queueing work", async () => {
    const created = manager.create({ name: "Verify", prompt: "", botId: "masque-1", enabled: false, verificationPending: true });
    const before = queued.length;
