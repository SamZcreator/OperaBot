...
//
// OperaBot owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "operabot/cua-local-vm";
export const IMAGE_LAYER_VERSION = "3";
export const IMAGE_LAYER_LABEL = "com.operabot.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "operabot-computer";
export const MANAGED_LABEL = "com.operabot.local-vm";
export const DRIVER_LABEL = "com.operabot.cua-driver";
export const BASE_IMAGE_LABEL = "com.operabot.cua-base";
export const WORKSPACE_LABEL = "com.operabot.workspace";
...
export const CUA_SOCKET = "/run/user/1000/operabot-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/operabot/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-operabot-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-operabot-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-operabot-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-operabot-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-operabot-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:operabot-cua-driver]' \\
      'command=/usr/local/bin/start-operabot-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by OperaBot; recreate it";
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared OperaBot derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/operabot-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "operabot-cua-image-"));
  try {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/operabot-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
/** Cloud boxes still use OperaBot's high-latency REST adapter. Local VMs
