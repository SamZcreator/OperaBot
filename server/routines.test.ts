...
describe("RoutineManager", () => {
  ...
  it("queues behind a busy bot, then dispatches into a detached task", async () => {
    ...
    h.setBot("ready");
    await h.manager.tick();
    expect(h.started).toEqual([{ botId: "masque-2", threadId: "thread-1", prompt: "Review the queue" }]);
    expect(h.manager.listRuns()[0]).toMatchObject({ status: "running", threadId: "thread-1" });
    expect(h.manager.activeRunForBot("masque-2")?.threadId).toBe("thread-1");
  ...
  it("snapshots and dispatches the selected execution machine", async () => {
    ...
    h.setNow(routine.nextRunAt!);
    await h.manager.tick();
    h.manager.update(routine.id, { runOn: "masque" });
    ...
    expect(h.manager.listRoutines()[0]).toMatchObject({ runOn: "masque" });
  });
  ...
  it("opens webhook jobs in the assigned bot's live chat", async () => {
    ...
    expect(h.started).toEqual([{ botId: "masque-webhook", threadId: "thread-1", prompt: "Handle ticket 42" }]);
    expect(h.runOns).toEqual(["cloud"]);
