...
const readinessProbe =
  ...
  "--screenshot-out-file /tmp/operabot-readiness.png";
const readinessRead = `docker exec ${CONTAINER} base64 -w0 /tmp/operabot-readiness.png`;
...
describe("Cua integration", () => {
  ...
  it("builds an exact, checksum-verified Cua Driver 0.20.0 image", () => {
    ...
    expect(dockerfile).toContain("prepare-operabot-workspace.sh");
  ...
  it("captures the preview through Cua Driver rather than xdotool or VNC", async () => {
    const screenshotCall =
      `${driverExec} call get_desktop_state {} --socket ${CUA_SOCKET} ` +
      "--screenshot-out-file /tmp/operabot-preview.png";
    const png = validPng;
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      ...
      [screenshotCall]: "{}\n",
      [`docker exec ${CONTAINER} base64 -w0 /tmp/operabot-preview.png`]: png.toString("base64"),
    });
