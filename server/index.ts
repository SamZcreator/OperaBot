// OperaBot server — the harness host. Clients hold no transports
...
async function startTurn(
  ...
  opts?: {
     ...
     * of merely mounting that VM's computer tools on the MASQUE's provider. */
    runOn?: RoutineRunOn;
  ...
  const persona = [
    `You are ${bot.name}, a personal bot in OperaBot.`,
  ...
  void (async () => {
    try {
      ...
      const wants = opts?.runOn === "cloud" ? "cloud" : bot.computer; // cloud routine overrides the MASQUE default
      ...
      if (wants === "vm") {
        ...
        if (!cua) throw new Error("CUA Driver is not ready for this computer — check permissions and restart OperaBot");
...
// ordered behind a busy MASQUE and gives webhook runs the same durable receipts.
...
try {
  ...
  console.log(`operabot webhook receiver on ${webhookIngress.baseUrl}`);
} catch (error) {
  webhookIngressError = error instanceof Error ? error.message : String(error);
  console.error(`operabot webhook receiver unavailable: ${webhookIngressError}`);
}
...
async function runGroupMemberTurn(
  ...
  const roster = group.memberIds
    ...
    .join(", ");
  const system = [
    `You are ${bot.name}, a bot in the room "${group.name}" in OperaBot.`,
    bot.title && `Role: ${bot.title}.`,
    bot.description && `About: ${bot.description}`,
...
function dispatchConnectorResume(entry: { botId: string; threadId: string; resumeKey: string; labels: string[] }) {
  ...
  const prompt = `OperaBot connection update: the user securely connected ${names}. Continue the task that paused for this connection. Do not ask them to connect it again.`;
  if (owner.bot.busy) {
  ...
  try {
    ...
    if (path.startsWith("/api/internal/")) {
      ...
      if (method === "POST" && path === "/api/internal/ask-bot") {
        ...
        const prefixed = `[Message from @${currentFrom.name}, another bot in this OperaBot workspace. Reply to them.]\n\n${message}`;
    ...
    // relay never has to expose the rest of OperaBot's control surface.
    ...
    if (method === "GET" && path === "/api/health") {
      return json(res, 200, { app: "operabot", pid: process.pid, static: Boolean(STATIC_DIR) });
    }
