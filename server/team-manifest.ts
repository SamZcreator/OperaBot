import type { GroupDefaultResponder, MasqueColor } from "./store.ts";

export const TEAM_MANIFEST_FORMAT = "operabot.team" as const;
export const TEAM_MANIFEST_VERSION = 1 as const;

const COLORS: readonly MasqueColor[] = [
  "green",
  "blue",
  "red",
  "orange",
...
export function parseTeamManifest(value: unknown): TeamManifestV1 {
  ...
  if (value.format !== TEAM_MANIFEST_FORMAT) throw new Error("This is not an OperaBot team file");
  ...
  const members = team.members.map((raw, index): TeamManifestMember => {
    ...
    if (typeof appearance.color !== "string" || !COLORS.includes(appearance.color as MasqueColor)) {
    ...
    return {
      ...
      appearance: {
        color: appearance.color as MasqueColor,
