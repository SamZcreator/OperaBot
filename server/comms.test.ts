...
describe("mentionedBots", () => {
  ...
  it("matches a tag at a word start, case-insensitively", () => {
    expect(mentionedBots("hey @nadia, look", peers).map((b) => b.id)).toEqual(["3"]);
    expect(mentionedBots("@Nadia first thing", peers).map((b) => b.id)).toEqual(["3"]);
  });
  ...
  it("dedupes repeats and collects multiple bots", () => {
    expect(mentionedBots("@Nadia and @New Bot and @Nadia", peers).map((b) => b.id)).toEqual(["3", "1"]);
  });
  it("ignores emails, hidden bots, and mid-word @", () => {
    expect(mentionedBots("mail nadia@nadia.dev please", peers)).toEqual([]);
  ...
  it("requires a word boundary at the end of the name", () => {
    ...
    expect(mentionedBots("@Nadiao is someone else", peers)).toEqual([]);
  });
});
...
describe("roomResponders", () => {
  ...
  it("lets explicit mentions override the configured lead", () => {
    expect(roomResponders("@Nadia take this", members, { kind: "member", botId: "atlas" })).toEqual([members[1]]);
  });
...
describe("comms e2e (fake ACP fleet)", () => {
  ...
  beforeAll(async () => {
    ...
    mkdirSync(join(home, ".operabot"), { recursive: true });
    writeFileSync(
