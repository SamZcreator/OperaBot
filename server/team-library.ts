...
export const TEAM_LIBRARY_REPOSITORY = "https://github.com/SamZcreator/operabot-teams";
export const TEAM_LIBRARY_RAW_ROOT = "https://raw.githubusercontent.com/SamZcreator/operabot-teams/main";
...
export function parseTeamCatalog(value: unknown): TeamCatalog {
  if (!isRecord(value) || value.format !== "operabot.catalog" || value.version !== 1) {
  ...
  const teams = value.teams.map((raw, index): TeamCatalogEntry => {
    ...
    return {
      ...
      manifest: relativeFile(raw.manifest, `${field}.manifest`, ".masqueteam.json", prefix),
...
export function githubManifestUrls(input: string): string[] {
  ...
  if (url.hostname === "github.com" || url.hostname === "www.github.com") {
    if (parts.length === 2) {
      ...
      return [
        `https://raw.githubusercontent.com/${owner}/${repo}/main/team.masqueteam.json`,
        `https://raw.githubusercontent.com/${owner}/${repo}/master/team.masqueteam.json`,
      ];
    }
...
export async function fetchGithubTeam(input: string, fetcher: Fetcher = fetch): Promise<ParsedTeamManifest> {
  ...
  throw lastError ?? new Error("No team.masqueteam.json file was found in that repository");
}
