...
const service: ServiceInfo = {
  name: "Nadia's computer",
  type: "_operabot._tcp",
  port: 8800,
  host: "operabot-1a2b3c4d.local",
  addresses: ["192.168.1.42"],
  txt: ["v=1", "name=Nadia's computer"],
};

const INSTANCE = "Nadia's computer._operabot._tcp.local";
const SERVICE_NAME = "_operabot._tcp.local";
...
describe("naming", () => {
  it("keeps an instance name to a single valid label", () => {
    ...
    expect(dnsLabel("")).toBe("OperaBot");
    expect(dnsLabel("   ")).toBe("OperaBot");
  });

  it("claims a host name the system responder will not fight us for", () => {
    const name = defaultHostName("Nadias-MacBook-Pro");
    expect(name).toMatch(/^operabot-[0-9a-f]{8}\.local$/);
    // stable across restarts, distinct per machine
    expect(defaultHostName("Nadias-MacBook-Pro")).toBe(name);
    expect(defaultHostName("another-machine")).not.toBe(name);
    expect(name).not.toContain("Nadias-MacBook-Pro");
  });
