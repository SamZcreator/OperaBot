...
description: Build, verify, and publish the Windows desktop build (NSIS installer + latest.yml) to the operabot-releases repo. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `OperaBot-<version>-setup.exe` and its update feed to
[SamZcreator/operabot-releases](https://github.com/SamZcreator/operabot-releases).
...
| File | Purpose |
|---|---|
| `OperaBot-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `OperaBot-<version>-setup.exe.blockmap` | differential updates |
| `OperaBot-<version>-x64.zip` | portable, not used by the updater |
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\OperaBot\logs\server.log`.
...
```powershell
Copy-Item release/OperaBot-<version>-setup.exe release/OperaBot-setup.exe
gh release upload v<version> --repo SamZcreator/operabot-releases `
  release/OperaBot-<version>-setup.exe `
  release/OperaBot-setup.exe `
  release/OperaBot-<version>-setup.exe.blockmap `
  release/latest.yml
```

Both names are required, for different consumers:

- **`OperaBot-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`OperaBot-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/OperaBot-setup.exe` button a stable URL. This
  mirrors `OperaBot.dmg` sitting beside `OperaBot-<version>.dmg`.
