...
ios/
  ...
  App/                           SwiftUI, and everything that needs a device
    ...
    Discovery.swift              NWBrowser for _operabot._tcp
    Keychain.swift               the device token
    MasqueAvatar.swift             the mascot face, in the desktop's palette
...
- **Messaging-app shape, not settings-list shape.** Mascot faces at roster size,
  ...
  stamp on every message. The palette in `MasqueAvatar.swift` is copied verbatim
...
through manual MagicDNS entry; it is not a dependency and OperaBot does not
