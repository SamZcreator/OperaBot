...
```sh
git clone https://github.com/SamZcreator/OperaBot
cd OperaBot
...
dns-sd -B _operabot._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/OperaBot must be allowed, or the phone reaches
...
cd ios && xcodegen generate && open OperaBotCompanion.xcodeproj
```
...
```sh
xcodebuild -project OperaBotCompanion.xcodeproj \
...
1. **Pair.** In OperaBot → Settings → Companion, choose **Set up a
   phone**. Scan the QR code with the phone's Camera, open OperaBotMobile,
   ...
   **Connect**. The computer should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → OperaBot → Local Network. If the
...
3. **In OperaBot → Settings → Companion:** with the toggle on, the panel now
...
- **Xcode console**, subsystem `com.operabot.companion`: `opening stream`,
