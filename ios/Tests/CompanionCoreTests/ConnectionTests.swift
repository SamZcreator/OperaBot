...
final class ConnectionTests: XCTestCase {
    ...
    func testParsesADesktopPairingInvite() throws {
        ...
        let url = try XCTUnwrap(URL(string: "operabot://pair?address=macbook.tail1234.ts.net%3A8810&token=\(token)&code=004209&name=Nadia%27s%20Mac"))
        ...
        XCTAssertEqual(invite.connection.name, "Nadia's Mac")
        XCTAssertEqual(invite.credential, token)
    }

    func testParsesAnOlderCodeOnlyPairingInvite() throws {
        let url = try XCTUnwrap(URL(string: "operabot://pair?address=mac.local&code=004209"))
    ...
    func testRejectsAnUntrustedOrMalformedPairingInvite() throws {
        ...
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "operabot://pair?address=mac.local&code=12345"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "operabot://pair?address=mac.local&token=weak"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "operabot://pair?address=mac.local&token=weak&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "operabot://pair?address=host%2Fpath&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "operabot://pair?address=one.local&address=two.local&code=123456"))))
    }
}
